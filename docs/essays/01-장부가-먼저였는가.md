...
```
[1차 사료 비문 라이덴 전사 (CDLI P254191)]
1. 𒁹 |NAM2.ŠEŠDA|  [실물 판독: 최고 통치자 / 제의적 군주 (나메시다)]
2. 𒁹 GAL:TE        [실물 판독: 신전 최고 재판관 / 집행관]
3. 𒁹 GAL:SUKKAL    [실물 판독: 최고 사절 / 총괄 행정관]
4. 𒁹 GAL:SANGA     [실물 판독: 신전 재정 총책임자]
5. 𒁹 GAL:DUB       [실물 판독: 수석 서기관장]
```

- **비문학적 고증**: 표제 기호 `|NAM2.ŠEŠDA|`는 지도자/결정권자를 뜻하는 `NAM2`와 제의적 권위를 상징하는 `ŠEŠDA`의 결합 글리프로, 세속 권력과 종교 권력이 미분화되었던 우루크 최고 지배자의 성격을 보여준다.
...
```
[원시 쐐기 행정 점토판 구조 분석]
[ 4(iku) GAN2 ] ➔ 토지 면적 계량 체계 (B-system, 약 1.4헥타르)
[ ŠE ]          ➔ 보리 작물 한정사 (사물 어휘 범주)
[ 1(dug) SILA3 ] ➔ 부피 용기 계량 체계 (Š-system, 정량 배분)
[ SANGA ]       ➔ 집행 신전 관료 직책 (직업목록 어휘와 1:1 일치)
```
